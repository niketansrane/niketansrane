 ---
  ...
  ---
  I use Azure Devops every day at my company. My team has a peculiar problem of work items and repos being in two different projects under the same organization. This makes sense when you think about the potential clientale being different for these two (the work items is more customer and PM friendly and repos being developer oriented) but it is pain to work with when you are switching context 20-30 times a day.
  ...
  I tried having a hard-coded config and it worked slightly better but the moment I had it interacting with work items in my existing coding session, it kept facing the same problem. Also, reminding LLM again and again that please use this config seemed like a waster of time and tokens.
  ...
  I didn't need a general- purpose ADO agen but rather a customizable one where my config could look like
  ...
  The MCP server is't designed for this level of customization. I needed something opinionated about my workflow.
  ...
  - Build slash commands that feel natural (inspired by compound-engineering plugin) /adoflow:workitems create a bug instead of wrestling with API parameters
  ...
  Right now it's just ado-flow and any team tool that requires persistent configuration and natural language workflows is a good candidate.
